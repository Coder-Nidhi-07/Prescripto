  ...
  return (
    <form className="min-h-[80vh] flex items-center ">
      <div className="flex flex-col gap-3  m-auto items-start p-8 min-w-[340px] am:min-w-96 border rounded-xl text-zinc-600 text-sm shadow-lg">
        ...
        <p>
          Please{state === "Sign up" ? "sign up" : " login"} login to book
          appointment
        </p>
        {
      ...
      {
        state==="Sign Up"
        ?<p>Already have an account? <span  onClick={()=>setState('Login')} className="text-[#5f6fff] underline cursor ">Login Here</span></p>
        :<p>Create a new account?<span onClick={()=>setState('Sign Up')} className="text-[#5f6fff] underline cursor "> click here</span></p>
      }
      </div>
    </form>
  );
